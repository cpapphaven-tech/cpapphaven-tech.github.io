...
function sendDurationOnExit(reason) {
    if (gameStartTime && !durationSent && window.trackGameEvent) {
        ...
        window.trackGameEvent(`game_duration_bubbleshooter_${seconds}_${reason}_${getBrowser()}`, {
            seconds,
            end_reason: reason,
            os: getOS(),
...
window.addEventListener("beforeunload", () => {
    ...
    sendDurationOnExit("tab_close_bubbleshooter");

    if (!gameStartedFlag && window.trackGameEvent) {
        const osKey = getOSKey();
        const placementId = getPlacementId();
        window.trackGameEvent(`exit_before_game_bubbleshooter_${osKey}`, {
            os: getOS(),
            placement_id: placementId
        });
