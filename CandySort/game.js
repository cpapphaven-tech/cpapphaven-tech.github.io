/* ============================================================
   Candy Sort: Sweet Color Puzzle – game.js
   Canvas-based game engine with custom Sweets
   ============================================================ */
...
(function () {
    ...
    // ── Constants ─────────────────────────────────────────────
    const MARBLE_R   = 16;      // candy radius (px)
    const BELT_SLOTS = 12;      // total slots on conveyor
    const BOX_SLOTS  = 3;       // candies per box
    const BELT_SPEED = 0.4;     // belt scroll speed (slots/sec)

    // Colors: [fill, stroke/shadow]
    const COLOR_PALETTE = [
        { id:'red',    fill:'#ff4d6d', shadow:'#c9184a', label:'🍬' }, // Peppermint
        { id:'blue',   fill:'#4cc9f0', shadow:'#0096c7', label:'🔷' }, // Wrapped candy
        { id:'green',  fill:'#4ade80', shadow:'#15803d', label:'🟢' }, // Lime slice
        { id:'yellow', fill:'#facc15', shadow:'#a16207', label:'🟡' }, // Lemon drop
        { id:'purple', fill:'#c084fc', shadow:'#7e22ce', label:'🟣' }, // Lollipop
        { id:'orange', fill:'#f97316', shadow:'#c2410c', label:'🟠' }, // Candy corn
    ];

    // ── Level Definitions ─────────────────────────────────────
    const LEVELS = [
        { colors:['red','yellow'],                  trays:3, boxTarget:4 },
        { colors:['red','yellow','green'],          trays:4, boxTarget:5 },
        { colors:['red','yellow','green','blue'],   trays:5, boxTarget:6 },
        { colors:['red','yellow','green','blue','purple'], trays:6, boxTarget:7 },
        { colors:['red','yellow','green','blue','purple','orange'], trays:6, boxTarget:8 },
        { colors:['red','yellow','green','blue','purple','orange'], trays:6, boxTarget:10 },
        { colors:['red','yellow','green','blue','purple'], trays:6, boxTarget:12 },
        { colors:['red','yellow','green','blue','purple','orange'], trays:6, boxTarget:14 },
    ];
    ...
    function loadSavedProgress() {
        try {
            const savedLevel = localStorage.getItem('pmg_candy_sort_level');
            const savedScore = localStorage.getItem('pmg_candy_sort_score');
            return {
    ...
    function saveProgress(level, score) {
        try {
            localStorage.setItem('pmg_candy_sort_level', level);
            localStorage.setItem('pmg_candy_sort_score', score);
        } catch (e) {}
    }
    ...
    function drawBeltFillBar() {
        ...
        let fillColor;
        if (ratio < 0.5)       fillColor = `rgba(244,114,182,${0.6 + ratio * 0.4})`;
    ...
    function drawTrays() {
        ...
        for (let i = 0; i < trayCount; i++) {
            ...
            ctx.fillStyle = isHint ? 'rgba(236,72,153,0.35)' : 'rgba(255,255,255,0.07)';
            ctx.fill();
            ctx.strokeStyle = isHint ? 'rgba(236,72,153,0.9)' : 'rgba(255,255,255,0.12)';
            ...
            for (let m = visibleCount - 1; m >= 0; m--) {
                ...
                drawCandy(r.cx, my, MARBLE_R, col);
            }
    ...
    function drawBelt() {
        ...
        beltGrad.addColorStop(0, '#260a3d');
        beltGrad.addColorStop(0.5, '#19062b');
        beltGrad.addColorStop(1, '#260a3d');
    ...
    function drawFallingMarbles() {
        for (const fm of state.fallingMarbles) {
            ...
            drawCandy(fm.x, fm.y, MARBLE_R, col);
        }
    }
    ...
    function drawCandy(x, y, r, col) {
        ...
        if (col.id === 'red') {
            ...
            for (let i = 0; i < wedges; i++) {
                const startAngle = (i * 2 * Math.PI) / wedges;
                const endAngle = startAngle + Math.PI / wedges;
                ctx.beginPath();
                ctx.moveTo(x, y);
            ...
            // Twist ends (left/right triangles)
            ctx.beginPath();
            ctx.moveTo(x - r, y);
            ctx.lineTo(x - r*1.3, y - r*0.4);
            ctx.lineTo(x - r*1.3, y + r*0.4);
            ctx.closePath();
            ctx.fill(); ctx.stroke();
            ...
            ctx.fillStyle = g;
            ctx.beginPath();
            ctx.ellipse(x, y, r*1.08, r*0.85, Math.PI*0.12, 0, Math.PI * 2);
            ctx.fill();

            // White sugar dust dots
            ctx.shadowBlur = 0;
            ctx.fillStyle = 'rgba(255,255,255,0.7)';
            for (let i = 0; i < 6; i++) {
                const dx = (Math.random() - 0.5) * r * 1.1;
                const dy = (Math.random() - 0.5) * r * 0.8;
                ctx.beginPath();
            ...
            // 🟠 Candy Corn (drawn as conic/triangle)
            ctx.shadowBlur = 0;
            ctx.beginPath();
            ctx.moveTo(x, y - r); // Top
            ctx.lineTo(x + r, y + r*0.9); // Bottom right
            ctx.lineTo(x - r, y + r*0.9); // Bottom left
            ctx.closePath();
            ctx.clip();

            // yellow base
            ctx.fillStyle = '#eab308';
            ctx.fillRect(x - r, y + r*0.3, r*2, r);
            ...
            // white tip
            ctx.fillStyle = '#ffffff';
            ctx.fillRect(x - r, y - r, r*2, r*0.7);
        }
        ...
        for (let i = 0; i < state.trays.length; i++) {
            ...
            for (let b = 0; b < state.boxes.length; b++) {
                ...
                if (!box.completed && box.color === topColor && box.slots.length < BOX_SLOTS) {
                    ...
                    if (hasBeltMarble) {
                        state.hintBox  = b;
                        showToast(`Wait for the ${topColor} candy to pass!`);
                        return;
                    } else {
                        state.hintTray = i;
                        showToast(`Drop the ${topColor} candy from the tray!`);
                        return;
                    }
                }
            }
