/**
 * Gem Shooter 3D - Mobile Optimized & Timer Version
...
let gems = []; // 2D array [row][col]
let shotGem = null;
let nextGem = null;
let score = 0;
let level = parseInt(localStorage.getItem('gem_shooter_current_level')) || 1;
let timeLeft = 60;
let timerInterval = null;
let gamesPlayed = 0;
let bestScore = localStorage.getItem('gem_shooter_best_score') || 0;
...
function sendDurationOnExit(reason) {
    if (gameStartTime && !durationSent && window.trackGameEvent) {
        ...
        window.trackGameEvent(`game_duration_gemshooter_${seconds}_${reason}_${getBrowser()}`, {
            seconds,
            end_reason: reason,
            os: getOS(),
...
document.addEventListener("visibilitychange", () => {
    if (document.hidden) {
        ...
        sendDurationOnExit("background_gem_shooter");
    }
});

window.addEventListener("beforeunload", () => {

    sendDurationOnExit("tab_close_gemshooter");

    if (!gameStartedFlag && window.trackGameEvent) {
        const osKey = getOSKey();
        const placementId = getPlacementId();
        window.trackGameEvent(`exit_before_game_gemshooter_${osKey}`, {
            os: getOS(),
            placement_id: placementId
        });
...
function createGrid() {
    // Clear existing
    gems.forEach(row => row.forEach(b => { if (b) scene.remove(b.mesh); }));
    gems = [];
...
function createGem(color, r, c) {
    const geo = new THREE.IcosahedronGeometry(BUBBLE_RADIUS, 0);
...
function prepareNextGem() {
    // Pick color that exists in the grid if possible
    let existingColors = new Set();
    gems.forEach(row => row.forEach(b => { if (b) existingColors.add(b.color); }));
    ...
    nextGem = new THREE.Mesh(geo, mat);
    nextGem.color = color;
    nextGem.position.copy(shooterPos);
    scene.add(nextGem);
}

function onPointerDown(e) {
    if (!gameStarted || isGameOver || !canShoot || !nextGem) return;
...
function animate() {
    ...
    if (shotGem) {
        shotGem.position.add(velocity);

        // Wall bounce
        if (Math.abs(shotGem.position.x) > 11) {
            velocity.x *= -1;
            shotGem.position.x = Math.sign(shotGem.position.x) * 11;
        }
...
function checkCollision() {
    ...
    if (shotGem.position.y > 12.5) { // Threshold adjusted for Y=12
        snapToGrid(shotGem);
        return;
    }
    ...
    for (let r = 0; r < GRID_ROWS; r++) {
        for (let c = 0; c < GRID_COLS; c++) {
            const b = gems[r][c];
            if (b) {
                const dist = shotGem.position.distanceTo(b.mesh.position);
    ...
    if (shotGem.position.y < -20 || shotGem.position.y > 20) {
        scene.remove(shotGem);
        shotGem = null;
        canShoot = true;
    }
}

function snapToGrid(gem) {
    let bestR = 0, bestC = 0, minDist = Infinity;
    for (let r = 0; r < GRID_ROWS; r++) {
        for (let c = 0; c < GRID_COLS; c++) {
            if (gems[r][c]) continue;
            if (r % 2 === 1 && c === GRID_COLS - 1) continue;
            const pos = getGridPosition(r, c);
            const dist = gem.position.distanceTo(new THREE.Vector3(pos.x, pos.y, 0));
            if (dist < minDist) { minDist = dist; bestR = r; bestC = c; }
        }
    }

    const finalPos = getGridPosition(bestR, bestC);
    gem.position.set(finalPos.x, finalPos.y, 0);
    const b = { mesh: gem, color: gem.color, r: bestR, c: bestC };
    gems[bestR][bestC] = b;
    shotGem = null;
...
function handleMatches(r, c) {
    const matches = findMatches(r, c, gems[r][c].color);
    if (matches.length >= 3) {
        matches.forEach(m => {
            scene.remove(m.mesh);
            gems[m.r][m.c] = null;
        ...
        setTimeout(dropFloatingGems, 50);
...
function findMatches(r, c, color, visited = new Set()) {
    ...
    visited.add(key);
    let found = [{ r, c, mesh: gems[r][c].mesh }];
    getNeighbors(r, c).forEach(n => {
        const b = gems[n.r][n.c];
...
function dropFloatingGems() {
    const connected = new Set();
    for (let c = 0; c < GRID_COLS; c++) { if (gems[0][c]) markConnected(0, c, connected); }
    for (let r = 1; r < GRID_ROWS; r++) {
        for (let c = 0; c < GRID_COLS; c++) {
            if (gems[r][c] && !connected.has(`${r},${c}`)) {
                scene.remove(gems[r][c].mesh);
                gems[r][c] = null;
                score += 20;
            }
        }
...
function markConnected(r, c, connected) {
    ...
    connected.add(key);
    getNeighbors(r, c).forEach(n => { if (gems[n.r][n.c]) markConnected(n.r, n.c, connected); });
}

function checkWin() {
    let count = 0;
    gems.forEach(row => row.forEach(b => { if (b) count++; }));
    if (count === 0) {
...
function nextLevel() {
    ...
    level++;
    localStorage.setItem('gem_shooter_current_level', level);
    levelEl.innerText = level;
    gameStarted = true;
    canShoot = true;
    createGrid();
    startTimer();
    if (!nextGem) prepareNextGem();
}
...
function gameOver(title) {
    ...
    if (score > bestScore) {
        bestScore = score;
        localStorage.setItem('gem_shooter_best_score', bestScore);
    }
