...
function startNeverEndingGame() {
	...
	function updateHUD() {
		...
		if (currentScoreEl) currentScoreEl.innerText = `Score: ${Math.floor(state.distanceTravelled)}`;
		if (highScoreEl) highScoreEl.innerText = `Best: ${Math.floor(state.highScore)}`;
	}
